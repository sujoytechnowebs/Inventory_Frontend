    ...
    <q-card-section>
      ...
      >
        <template v-slot:top>
          ...
          >
            Stock Transfer Management Table
          </div>
...
export default defineComponent({
  ...
  computed: {
    ...mapFields("stocktransfer", ["filter.search", "filter"]),
  },
  setup() {
    const { getGetters } = useStoreModule();
    const { showEditModal } = getGetters("stocktransfer", ["showEditModal"]);
    const { showCreateModal } = getGetters("stocktransfer", [
      "showCreateModal",
    ]);
